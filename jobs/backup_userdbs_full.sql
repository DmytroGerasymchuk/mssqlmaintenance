...
declare @JobName sysname = N'$(MaintDBName): Backup: User DBs: Full'
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'backup', 
		...
		@command=N'execute $(MaintDBName).maint.usp_backup ''*user''', 
		@database_name=N'master', 
		@output_file_name=N'$(JobLogDir)\backup-user-dbs-full.txt', 
		@flags=4
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
