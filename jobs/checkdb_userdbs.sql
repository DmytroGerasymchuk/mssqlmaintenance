...
declare @JobName sysname = N'$(MaintDBName): Check Database (User DBs)'
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'proceed', 
		...
		@command=N'execute $(MaintDBName).maint.usp_db_check ''*user''', 
		@database_name=N'master', 
		@output_file_name=N'$(JobLogDir)\db-check-user-dbs.txt', 
		@flags=4
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
